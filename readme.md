...
The best practice method for me would be to extract the product into separate classes, one for physical and one for digital, this means each one can have its own set of attributes and methods if needed, but these would always extend a base class which contains methods relevant and accessible to both.
